 ...
 */
public class ConsumerOldClientTestSkip extends BaseRequestTest {

    private static Logger log = LoggerFactory.getLogger(ConsumerOldClientTestSkip.class);
    ...
    private static class ClientTask implements Runnable {
        ...
        public void run() {
            while (true) {
                try {
                    byte[] fetchRequestBytes = messageDeSer.serializeRequest(ClientServerSpec.API_KEY_FETCH_REQUEST,
                            ClientServerSpec.API_VERSION_1,
                            ClientServerSpec.COMPRESSION_CODEC_SNAPPY,
                            fetchRequest);

                    out.write(fetchRequestBytes);
                    out.flush();
                    ...
                    GenericRecord responseRecord =
                            messageDeSer.deserializeResponse(apiKeyAvroSchemaMap.getSchemaName(ClientServerSpec.API_KEY_FETCH_RESPONSE), totalSize, ByteBuffer.wrap(responseMessageBytes));
